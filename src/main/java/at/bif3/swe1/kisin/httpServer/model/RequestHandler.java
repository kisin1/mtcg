...
import at.bif3.swe1.kisin.httpServer.databank.TradingsDAO;
...
import at.bif3.swe1.kisin.monsterTradingCards.Battle;
import at.bif3.swe1.kisin.monsterTradingCards.Trade;
    ...
    public void run() {
        ...
        try {
            ...
            if(players.size() == 2){
                ...
                battle.start();
                response = new Response(StatusCode.OK, ContentType.TEXT_PLAIN, "Battle has started");
            }
...
//            out.write(new Response(StatusCode.OK, ContentType.APPLICATION_JSON, "hola" + request.getBody()).get());
    ...
    private void handleResponse() throws IOException {
        ...
        switch (path){
           ...
           case "battles" -> response = new Response(StatusCode.NOT_IMPLEMENTED, ContentType.TEXT_PLAIN, "Battle not implemented");//handleBattle(username);
           case "tradings" -> handleTradings(username);
    ...
    private void handleDeck(String username){
        switch (request.getMethod()){
            ...
            default -> response = new Response(StatusCode.NOT_IMPLEMENTED, ContentType.TEXT_PLAIN, "Method not implemented for deck");
        }
    }
            ...
            if(cardDAO.getDeck(username).isEmpty()){
                ...
                if(request.getPath().size() > 1){
                    if (request.getPath().get(1).equals("format=plain")){
                        try {
                            response = new Response(StatusCode.OK, ContentType.TEXT_PLAIN, username + " deck: \n" + Json.toJsonString(deck));
    ...
    private void handleBattle(String username){
        if(request.getMethod().equals(Method.POST)){
            initPlayer(username);
//            if(players.size() < 2){
//                response = new Response(StatusCode.OK, ContentType.TEXT_PLAIN, "One more player needed for a battle");
//            } else {
//                Battle battle = new Battle(players.get(0), players.get(1), 100);
//                battle.start();
//                response = new Response(StatusCode.OK, ContentType.TEXT_PLAIN, "Battle has started");
        ...
        if(isSignedIn(username)){
            if(request.getPath().size() < 2){
                switch (request.getMethod()){
                    ...
                    default -> response = new Response(StatusCode.NOT_IMPLEMENTED, ContentType.TEXT_PLAIN, "Method not implemented");
                }
            } else {
                switch (request.getMethod()){
